...
export async function syncGfw(supabase, apiKey, opts = {}) {
  const { limit, perVessel = 5, backfill = false, datasets = ['ports', 'loitering'], log = console.log } = opts
  ...
  const toGeocode = limit ? (latest ?? []).filter((r) => vessels.some((v) => v.id === r.vessel_id)) : (latest ?? [])
