...
**Campaign A (Brand) BUILT + LIVE 2026-09-03** — `brand-campaign` (ID 24208064628), published Enabled: Clicks + $2 cap, $329/day, same 15 countries, Presence-only, networks off, AI Max off, EU-political No. Two ad groups — *VesselConnect* ([vesselconnect], [vessel connect], "vesselconnect research vessels") and *Greenwater* ([greenwater foundation], "greenwater marine sciences") — 2 hand-written RSAs each (4/4), landing on the homepage; "Global negatives" attached. Build-day gotchas, again: the identity wall reappeared mid-wizard and ate the EU-political answer AND the entire keywords/ads step (recovered via draft "Finish" + full re-entry — after ANY wall, re-verify every step, and never URL-hack currentStep, it breaks the SPA); the new-ad-group flow prefilled broad junk keywords AND a scraped RSA quoting the same competitor catamaran (Clear prefills did nothing this time — manually overwrote/cleared every field). Note: UI shows "Sitelinks are missing from this campaign" — account-level sitelinks exist and should serve; verify inheritance later. Remaining: campaigns C (Discovery) + D (Operators).

**Keyword broadening (2026-09-02):** three weeks post-launch showed ~zero delivery (1 impression), 8 of 10 keywords dormant on low search volume — the net was too tight. Added 14 intent-bearing phrase-match keywords (account now 24): RVC group +5 ("charter a research boat", "research boat rental", "research ship charter", "rent a research vessel", "research vessel rental"), Ocean group +4 ("oceanographic ship charter", "hire a research ship", "scientific vessel charter", "marine research boat hire"), Survey group +5 ("survey boat charter", "hydrographic survey vessel charter", "boat for field research", "expedition vessel charter", "vessel for scientific expedition"). All under review at add time; verified 24/24 persisted on fresh load (one batch was lost to a browser-extension drop mid-save and re-added — the count check caught it). Bare "research vessel" still held as last resort. NOTE: account shows a "Complete advertiser verification" banner — Google identity verification needs Adam; ignoring it can eventually pause serving.
